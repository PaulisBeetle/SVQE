...
heisenberg_ij(nbit::Int, m::Int, n::Int) = put(nbit,m=>X) * put(nbit,n=>X) + put(nbit,m=>Y) * put(nbit,n=>Y) + put(nbit,m=>Z) * put(nbit,n=>Z)
...
function energy(circuit,model::Heisenberg;nbatch = 1024)
    ...
    nextnearbonds = get_nextnear_bonds(model)
    en = sum(basis->ising_energy(circuit,bonds,basis;nbatch = nbatch),[X,Y,Z])
    en += sum(basis->ising_energy(circuit,nextnearbonds,basis;nbatch = nbatch),[X,Y,Z])*0.4
end
...
function train(circuit, model; m = 5, VG = nothing, maxiter = 5, nbatch = 1024)
...
m = 3;
maxiter = 3;
loss_history,mycircuit = train(mycircuit,model,m=m,maxiter=maxiter,VG = VG)
pic = plot([0:npara*(maxiter+1)-1],[loss_history,fill(EG,npara*(maxiter+1))],label = ["QMPS","Exact"],lw = 3,ylabel = "Energy")
savefig("D:\\")
